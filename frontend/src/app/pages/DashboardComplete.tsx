...
export function DashboardComplete() {
  ...
  return (
    <div className={`min-h-screen relative overflow-hidden transition-colors ${theme === 'dark'
      ? 'bg-gradient-to-br from-[#1a1512] via-[#231c17] to-[#2d241d]'
      : 'bg-gradient-to-br from-[#c4b5a0] via-[#b8a590] to-[#a89780]'
      }`}>
      {/* Subtle Background Texture */}
      <div className="fixed inset-0 opacity-40">
        <div className={`absolute top-0 left-0 w-[800px] h-[800px] bg-gradient-radial blur-[100px] ${theme === 'dark' ? 'from-[#c9983a]/10 to-transparent' : 'from-[#d4c4b0]/30 to-transparent'
          }`} />
        <div className={`absolute bottom-0 right-0 w-[900px] h-[900px] bg-gradient-radial blur-[120px] ${theme === 'dark' ? 'from-[#c9983a]/5 to-transparent' : 'from-[#b8a898]/20 to-transparent'
          }`} />
      </div>
      ...
      {/* Sidebar */}
      <aside className={`fixed top-4 bottom-4 z-[101] transition-all duration-300 shadow-2xl lg:shadow-none ${isMobile
        ? `left-4 w-[280px] ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-[110%]'}`
        : `left-4 ${isSidebarCollapsed ? 'w-[93px]' : 'w-72'}`
        }`}>
        {/* Toggle Arrow Button - hidden on mobile */}
        {!isMobile && (
          <button
            onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)}
            className={`absolute z-[100] backdrop-blur-[90px] rounded-full border-[0.5px] w-8 h-8 shadow-md hover:shadow-lg transition-all flex items-center justify-center ${isSidebarCollapsed ? '-right-4 top-[60px]' : '-right-4 top-[60px]'
              } ${theme === 'dark'
        ...
        <div className={`h-full backdrop-blur-[90px] rounded-[29px] border shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)] relative overflow-y-auto scrollbar-hide transition-colors ${theme === 'dark'
          ...
          <div className="flex flex-col h-full px-0 py-[40px]">
            ...
            <div className={`flex items-center mb-6 transition-all ${(isSidebarCollapsed && !isMobile) ? 'px-[18px] justify-center' : 'px-6 justify-between lg:justify-start'
              ...
              <div className="flex items-center space-x-3">
                ...
                {(!isSidebarCollapsed || isMobile) && (
                  <span className={`text-[18px] lg:text-[20px] font-bold transition-colors ${theme === 'dark' ? 'text-[#f5efe5]' : 'text-[#2d2820]'
                    }`}>Grainlify</span>
                )}
              </div>
              {isMobile && (
                <button
                  onClick={() => setIsMobileMenuOpen(false)}
                  className={`p-2 rounded-full ${theme === 'dark' ? 'bg-white/5 hover:bg-white/10' : 'bg-black/5 hover:bg-black/10'}`}
                >
              ...
              {navItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`group w-full flex items-center rounded-[12px] transition-all duration-300 ${(isSidebarCollapsed && !isMobile) ? 'justify-center px-0 py-4' : 'justify-start px-3 py-2.5'
                      } ${isActive
                  ...
                  >
                    <item.icon className={`w-6 h-6 transition-colors ${(isSidebarCollapsed && !isMobile) ? '' : 'flex-shrink-0'} ${isActive ? 'text-white' : theme === 'dark' ? 'text-[#e8c77f]' : 'text-[#a2792c]'
                      }`} />
                    {(!isSidebarCollapsed || isMobile) && (
                      <span className={`ml-3 font-medium text-[14px] ${isActive ? 'text-white' : theme === 'dark' ? 'text-[#d4c5b0]' : 'text-[#6b5d4d]'
                        }`}>
                        {item.label}
                      </span>
                    )}
            ...
            <div className={`mt-auto ${isSidebarCollapsed ? 'px-[18px]' : 'px-6'} pt-6 space-y-2`}>
              <button
                ...
                className={`w-full flex items-center rounded-[12px] transition-all py-4 hover:bg-white/[0.1] ${isSidebarCollapsed ? 'justify-center px-0' : 'justify-start px-3'
                  } ${theme === 'dark' ? 'text-[#d4af37]' : 'text-[#a2792c]'
                  }`}
                title={isSidebarCollapsed ? 'Settings' : ''}
              >
                <Settings className={`w-6 h-6 ${isSidebarCollapsed ? '' : 'flex-shrink-0'}`} />
                {!isSidebarCollapsed && <span className={`ml-3 font-medium text-[14px] ${theme === 'dark' ? 'text-[#b8a898]' : 'text-[#6b5d4d]'
                  }`}>Settings</span>}
              </button>
      ...
      {/* Main Content */}
      <main className={`mr-4 my-4 relative z-10 transition-all duration-300 ${isMobile ? 'ml-4' : (isSidebarCollapsed ? 'ml-[117px]' : 'ml-80')}`}>
        <div className="max-w-[1400px] mx-auto">
          {/* Top Bar */}
          <div className={`backdrop-blur-[40px] rounded-[24px] border shadow-[0_8px_32px_rgba(0,0,0,0.08)] p-4 lg:p-6 mb-6 transition-colors overflow-visible flex items-center gap-4 ${theme === 'dark'
            ...
            <div className="flex-1 flex items-center justify-between">
              ...
              <div className="flex-1 max-w-lg">
                <div className="relative">
                  <Search className={`absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 transition-colors ${theme === 'dark' ? 'text-[#b8a898]' : 'text-[#7a6b5a]'
                    }`} />
                  <input
                    type="text"
                    placeholder="Search projects, issues, contributors..."
                    className={`w-full pl-12 pr-4 py-3.5 rounded-[14px] backdrop-blur-[30px] border focus:outline-none transition-all text-[14px] ${theme === 'dark'
              ...
              <div className="flex items-center space-x-3 relative z-50">
                ...
                <button
                  onClick={toggleTheme}
                  className={`p-3 rounded-[14px] backdrop-blur-[30px] border transition-all shadow-[0_4px_12px_rgba(0,0,0,0.04)] ${theme === 'dark'
                ...
                <button className={`relative p-3 rounded-[14px] backdrop-blur-[30px] border transition-all shadow-[0_4px_12px_rgba(0,0,0,0.04)] ${theme === 'dark'
                  ? 'bg-white/[0.08] border-white/15 text-[#d4af37] hover:bg-white/[0.12]'
                  : 'bg-white/[0.15] border-white/25 text-[#7a6b5a] hover:bg-white/[0.2]'
                  }`}>
          ...
          {currentPage === 'admin' && !adminAuthenticated && (
            <div className="flex items-center justify-center h-full">
              <div className={`text-center p-8 rounded-[24px] backdrop-blur-[40px] border ${theme === 'dark'
      ...
      >
        <form onSubmit={handleAdminPasswordSubmit}>
          <div className="space-y-4">
            <p className={`text-sm ${theme === 'dark' ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'
              }`}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`backdrop-blur-[40px] rounded-[24px] border shadow-[0_8px_32px_rgba(0,0,0,0.08)] p-8 transition-colors ${theme === 'dark'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project) => (
            <div
              key={project.id}
              className={`backdrop-blur-[30px] rounded-[20px] border p-6 transition-all cursor-pointer ${theme === 'dark'
      ...
      <div className={`backdrop-blur-[40px] rounded-[24px] border shadow-[0_8px_32px_rgba(0,0,0,0.08)] p-8 transition-colors ${theme === 'dark'
        ...
        <div className="mb-6">
          <h3
            className={`text-[24px] font-bold transition-colors ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
              }`}
          >
            Recommended Issues
          </h3>
          <p
            className={`text-[14px] mt-1 transition-colors ${theme === 'dark' ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'
              }`}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 items-stretch">
          {/* Issue Card */}
          <div
            className={`backdrop-blur-[30px] rounded-[16px] border p-5 transition-all h-full min-h-[180px] flex flex-col ${theme === 'dark'
          ...
          >
            ...
            <div className="flex items-start justify-between mb-3 gap-3 min-h-[3.5rem]">
              <h4
                className={`text-[16px] font-semibold leading-snug line-clamp-2 ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
                  }`}>
                Add support for new React hooks
              </h4>
              <span
                className={`shrink-0 px-2.5 py-1 rounded-[8px] text-[11px] font-semibold whitespace-nowrap ${theme === 'dark'
            ...
            {/* Footer */}
            <div
              className={`flex items-center justify-between text-[12px] mt-auto ${theme === 'dark' ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'
                }`}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className={`text-[32px] font-bold mb-2 transition-colors ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
            }`}>Open-Source Week</h1>
          <p className={`text-[16px] transition-colors ${theme === 'dark' ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'
            }`}>
      ...
      <div className="space-y-5">
        {events.map((event) => (
          <div
            key={event.id}
            className={`backdrop-blur-[40px] rounded-[24px] border p-8 shadow-[0_8px_32px_rgba(0,0,0,0.08)] transition-colors ${theme === 'dark'
              ? 'bg-white/[0.08] border-white/10'
          ...
          >
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-start space-x-4">
                ...
                <div>
                  <h3 className={`text-[22px] font-bold mb-2 transition-colors ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
                    }`}>{event.title}</h3>
                  <span className={`px-3 py-1.5 rounded-[10px] text-[12px] font-semibold ${theme === 'dark'
                    ? 'bg-green-500/30 border border-green-500/50 text-green-300'
                    : 'bg-green-500/20 border border-green-600/30 text-green-800'
                    }`}>
                    {event.status}
            ...
            <div className="grid grid-cols-4 gap-8 mb-6">
              <div>
                <div className={`text-[12px] mb-2 transition-colors ${theme === 'dark' ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'
                  }`}>Contributors</div>
                <div className={`text-[28px] font-bold transition-colors ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
              ...
              <div>
                ...
                <div className={`text-[28px] font-bold flex items-center space-x-2 transition-colors ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
                  }`}>
            ...
            <div className="flex items-center justify-between pt-6 border-t border-white/10">
              <div className="grid grid-cols-2 gap-8">
                <div>
                  <div className={`text-[12px] mb-1 transition-colors ${theme === 'dark' ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'
                    }`}>Start date</div>
                  <div className={`text-[15px] font-semibold transition-colors ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
                    }`}>{event.startDate}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {pastEvents.map((event) => (
          <div
            key={event.id}
            className={`backdrop-blur-[40px] rounded-[20px] border p-6 shadow-[0_8px_32px_rgba(0,0,0,0.08)] transition-all ${theme === 'dark'
          ...
          >
            <div className="flex items-start space-x-3 mb-5">
              ...
              <div>
                <h4 className={`text-[18px] font-bold transition-colors ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
                  }`}>{event.title}</h4>
                <h5 className={`text-[14px] transition-colors ${theme === 'dark' ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'
                  }`}>{event.subtitle}</h5>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 mb-5">
              <div>
                <div className={`text-[16px] font-bold transition-colors ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
                  }`}>{event.contributors}</div>
                <div className={`text-[11px] transition-colors ${theme === 'dark' ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'
                  }`}>Contributors</div>
              </div>
              <div>
                <div className={`text-[16px] font-bold transition-colors ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
            ...
            <div className={`flex items-center justify-between text-[12px] pt-4 border-t border-white/10 transition-colors ${theme === 'dark' ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'
              }`}>
              <div>
...
function EcosystemsPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header Section */}
      <div className={`backdrop-blur-[40px] bg-gradient-to-br rounded-[28px] border shadow-[0_8px_32px_rgba(0,0,0,0.08)] p-10 transition-colors ${theme === 'dark'
        ? 'from-white/[0.08] to-white/[0.04] border-white/10'
        : 'from-white/[0.15] to-white/[0.08] border-white/20'
        }`}>
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <h1 className={`text-[36px] font-bold mb-3 transition-colors ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
      ...
      {/* Search Bar Section */}
      <div className={`backdrop-blur-[40px] rounded-[20px] border shadow-[0_4px_16px_rgba(0,0,0,0.06)] p-5 transition-colors ${theme === 'dark'
        ? 'bg-white/[0.08] border-white/10'
        : 'bg-white/[0.12] border-white/20'
        }`}>
        <div className="relative">
          <Search className={`absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 z-10 pointer-events-none transition-colors ${theme === 'dark' ? 'text-[#c9983a]' : 'text-[#8b6f3a]'
            }`} />
          <input
            type="text"
            ...
            className={`w-full pl-12 pr-4 py-3.5 rounded-[14px] backdrop-blur-[30px] border focus:outline-none transition-all text-[14px] shadow-[inset_0px_0px_4px_0px_rgba(0,0,0,0.12)] relative ${theme === 'dark'
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 animate-pulse">
          {Array.from({ length: 6 }).map((_, idx) => (
            <div
              key={idx}
              className={`backdrop-blur-[30px] rounded-[20px] border p-6 ${theme === 'dark'
                ? 'bg-white/[0.08] border-white/10'
                : 'bg-white/[0.15] border-white/25'
                }`}
            >
              <div className="flex items-start justify-between mb-5">
                <div
                  className={`w-14 h-14 rounded-[14px] ${theme === 'dark' ? 'bg-white/10' : 'bg-black/10'
                    }`}
                />
              </div>
              <div
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredEcosystems.map((ecosystem) => {
            ...
            return (
              <div
                key={ecosystem.id}
                className={`backdrop-blur-[30px] rounded-[20px] border p-6 transition-all cursor-pointer group ${theme === 'dark'
              ...
              >
                ...
                <div className="flex items-center gap-6 mb-4">
                  <div>
                    <div className={`text-[11px] mb-0.5 transition-colors ${theme === 'dark' ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'
                      }`}>Projects</div>
                ...
                {/* Description */}
                <p className={`text-[13px] mb-5 leading-relaxed transition-colors ${theme === 'dark' ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'
                  }`}>
                ...
                {ecosystem.website_url && (
                  <div className="flex items-center gap-2 mt-4">
                    ...
                    <a
                      ...
                      className={`text-[12px] hover:underline transition-colors ${theme === 'dark' ? 'text-[#c9983a]' : 'text-[#8b6f3a]'
                        }`}
                    >
                      Visit Website
                    </a>
      ...
      <div className={`backdrop-blur-[40px] bg-gradient-to-br rounded-[24px] border shadow-[0_8px_32px_rgba(0,0,0,0.08)] p-10 transition-all overflow-hidden relative ${theme === 'dark'
        ...
        <div className="relative z-10">
          <div className="text-center max-w-2xl mx-auto">
            ...
            <h3 className={`text-[28px] font-bold mb-4 transition-colors ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
              }`}>Missing Your Ecosystem?</h3>

            <p className={`text-[16px] mb-6 leading-relaxed transition-colors ${theme === 'dark' ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'
              }`}>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[10000]" onClick={() => setShowAddModal(false)}>
          <div
            className={`backdrop-blur-[40px] rounded-[24px] border shadow-[0_8px_32px_rgba(0,0,0,0.12)] p-8 w-[500px] max-w-[90vw] transition-colors ${theme === 'dark'
              ? 'bg-white/[0.12] border-white/20'
              : 'bg-white/[0.15] border-white/25'
              }`}
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className={`text-[24px] font-bold mb-6 transition-colors ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className={`w-full px-4 py-3 rounded-[14px] backdrop-blur-[30px] border focus:outline-none transition-all text-[14px] resize-none ${theme === 'dark'
                      ? 'bg-white/[0.08] border-white/15 text-[#f5f5f5] placeholder-[#d4d4d4] focus:bg-white/[0.12] focus:border-[#c9983a]/30'
                      : 'bg-white/[0.15] border-white/25 text-[#2d2820] placeholder-[#7a6b5a] focus:bg-white/[0.2] focus:border-[#c9983a]/30'
                      }`}
              ...
              <div className="flex items-center justify-end gap-3 mt-6">
                <button
                  ...
                  className={`px-5 py-2.5 rounded-[12px] backdrop-blur-[20px] border font-medium text-[14px] transition-all ${theme === 'dark'
      ...
      {showRequestModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[10000]" onClick={() => setShowRequestModal(false)}>
          <div
            className={`backdrop-blur-[40px] rounded-[24px] border shadow-[0_8px_32px_rgba(0,0,0,0.12)] p-8 w-[550px] max-w-[90vw] max-h-[90vh] overflow-y-auto transition-colors ${theme === 'dark'
              ? 'bg-white/[0.12] border-white/20'
              : 'bg-white/[0.15] border-white/25'
              }`}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-start justify-between mb-6">
              ...
              <button
                ...
                className={`p-2 rounded-[10px] transition-all hover:scale-110 flex-shrink-0 ml-4 ${theme === 'dark'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`backdrop-blur-[40px] rounded-[24px] border shadow-[0_8px_32px_rgba(0,0,0,0.08)] p-8 transition-colors ${theme === 'dark'
        ...
        {/* Info Message */}
        <div className={`backdrop-blur-[30px] rounded-[16px] border p-5 flex items-start gap-4 transition-colors ${theme === 'dark'
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[10000] animate-in fade-in" onClick={() => setShowAddModal(false)} style={{ backdropFilter: 'blur(8px)' }}>
          <div
            className={`backdrop-blur-[40px] rounded-[24px] border shadow-[0_8px_32px_rgba(0,0,0,0.12)] p-8 w-[550px] max-w-[90vw] transition-all animate-in zoom-in-95 ${theme === 'dark'
              ? 'bg-white/[0.12] border-white/20'
              : 'bg-white/[0.15] border-white/25'
              }`}
            onClick={(e) => e.stopPropagation()}
