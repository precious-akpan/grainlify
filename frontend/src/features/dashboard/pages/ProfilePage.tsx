...
export function ProfilePage({
  ...
  const effectiveViewingUserLogin = username || viewingUserLogin;
  const effectiveViewingUserId = viewingUserId;
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        } else if (user?.github?.login) {
          // Fetch own profile only if logged in
    ...
    fetchProfile();
  }, [effectiveViewingUserId, effectiveViewingUserLogin, user?.github?.login]);
  ...
  const activeLanguages =
    profileData?.languages?.slice(0, 3).map((lang) => {
      const maxCount = Math.max(
        ...(profileData.languages?.map((l) => l.contribution_count) || [0]),
      );
      return {
        name: lang.language,
  ...
  const activeEcosystems =
    profileData?.ecosystems?.slice(0, 2).map((eco) => {
      const maxCount = Math.max(
        ...(profileData.ecosystems?.map((e) => e.contribution_count) || [0]),
      );
      return {
  ...
  // Helper function to check if activity matches search query
  const matchesSearchQuery = (activity: any, query: string): boolean => {
    if (!query.trim()) return true; // If query is empty, show all
    ...
    if (activity.project_name.toLowerCase().includes(lowerQuery)) {
      return true;
    }

    // Check issue/PR number (as string)
    if (activity.number.toString().includes(lowerQuery)) {
      return true;
    }
          ...
          <div className="relative flex items-start justify-between gap-10">
            ...
            <div className="flex items-start gap-7">
                ...
                {isLoadingProfile ? (
                  <>
                    <SkeletonLoader
                      ...
                      width="128px"
                      height="128px"
                      className="border-[6px] border-white/40"
                    />
                    <SkeletonLoader
                      variant="circle"
                      width="48px"
              ...
              <div className="flex-1 pt-1">
                ...
                {/* Bio and Website */}
                {!isLoadingProfile &&
                  (profileData?.bio || profileData?.website) && (
                ...
                {!isLoadingProfile && (
                  <div className="flex items-center gap-3 flex-wrap mb-4">
                    ...
                    {profileData?.telegram ? (
                      ...
                      >
                        <svg
                          className={`w-4 h-4 ${theme === "dark" ? "fill-[#9ca3af]" : "fill-[#6b7280]"}`}
                          viewBox="0 0 24 24"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
                      ...
                      <a
                        href={
                          profileData.linkedin.startsWith("http")
                            ? profileData.linkedin
                            : `https://www.linkedin.com/in/${profileData.linkedin.replace(/^@/, "").replace(/^in\//, "")}`
                        }
                        target="_blank"
                        rel="noopener noreferrer"
              ...
              <div className="relative backdrop-blur-[40px] rounded-[28px] border-[3.5px] border-white/50 shadow-[0_15px_60px_rgba(201,152,58,0.5),inset_0_2px_4px_rgba(255,255,255,0.5),0_0_60px_rgba(255,215,0,0.2)] p-10 min-w-[200px] text-center group-hover/rank:scale-105 group-hover/rank:shadow-[0_20px_80px_rgba(201,152,58,0.6),inset_0_2px_4px_rgba(255,255,255,0.6)] transition-all duration-500 bg-gradient-to-br from-[#c9983a]/40 via-[#d4af37]/30 to-[#c9983a]/25">
                ...
                <div className="relative mb-3">
                  {isLoadingProfile ? (
                    ...
                    <div
                      className="text-[64px] font-black bg-gradient-to-b from-[#1a1410] via-[#2d2820] to-[#c9983a] bg-clip-text text-transparent leading-none drop-shadow-[0_4px_12px_rgba(0,0,0,0.2)]"
                      style={{ letterSpacing: "-0.02em" }}
                    >
                      {profileData.rank.position}
                      <span className="text-[36px] align-super">
                        {profileData.rank.position === 1
                          ? "st"
                          : profileData.rank.position === 2
                            ? "nd"
                            : profileData.rank.position === 3
                              ? "rd"
                              : "th"}
                      </span>
                    </div>
          ...
          <div className="relative grid grid-cols-3 gap-5">
            {isLoadingProjects ? (
              ...
              projects.map((project, idx) => {
                const projectName =
                  project.github_full_name.split("/")[1] ||
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 mb-4 text-[13px]">
                      ...
                      >
                        ...
                        <span>
                          {(project.contributors_count || 0).toLocaleString()}
                        </span>
                      </div>
                      <div
                        className={`flex items-center gap-1.5 group-hover/project:text-[#c9983a] transition-colors ${theme === "dark" ? "text-[#d4d4d4]" : "text-[#7a6b5a]"
                      ...
                      >
                        ...
                        <span>
                          {(project.forks_count || 0).toLocaleString()}
                        </span>
                      </div>
                    </div>
              ...
              <div
                className={`col-span-3 text-center py-8 ${theme === "dark" ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}
              >
          ...
          <div className="backdrop-blur-[40px] bg-white/[0.12] rounded-[24px] border border-white/20 shadow-[0_8px_32px_rgba(0,0,0,0.08)] p-6">
            ...
            <div className="space-y-3">
              {isLoadingProfile ? (
                ...
                <div
                  className={`text-center py-4 ${theme === "dark" ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}
                >
                  No languages found
                </div>
              )}
              ...
              <div className="relative group/chart">
                ...
                <div className="w-[240px] h-[240px] relative">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                    <div className="text-[13px] font-bold text-[#7a6b5a] uppercase tracking-wider mb-1 animate-pulse">
                      Total
                    </div>
                    ...
                    <div className="text-[11px] font-semibold text-[#7a6b5a] mt-1">
                      USD Earned
                    </div>
                  </div>
                </div>
                ...
                {rewardsData.map((item, idx) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      />
                      <div className="flex-1">
                        <div className="text-[13px] font-semibold text-[#2d2820] mb-1 group-hover/card:text-[#c9983a] transition-colors">
                          {item.name}
                        </div>
                        <div className="flex items-baseline gap-2">
                          ...
                          <div className="text-[11px] font-bold text-[#c9983a] group-hover/card:scale-110 transition-transform">
                            {item.value}%
                          </div>
                        </div>
                      </div>
              ...
              {isLoadingCalendar ? (
                <div className="flex-1 flex justify-between gap-[3px]">
                  {Array.from({ length: 52 }).map((_, weekIdx) => (
                    <div
                      key={weekIdx}
                      className="flex flex-col gap-[3px] flex-1 max-w-[20px]"
                    >
                  ...
                  {Array.from({ length: 52 }).map((_, weekIdx) => (
                    ...
                    >
                      {Array.from({ length: 7 }).map((_, dayIdx) => {
                        ...
                        const calendarEntry = contributionCalendar.find(
                        ...
                        if (level === 1) {
                          bgColor =
                            "bg-[#c9983a]/50 border-2 border-[#c9983a]/70";
                          shadowClass =
                            "shadow-[0_2px_10px_rgba(201,152,58,0.3)]";
                        } else if (level === 2) {
                          bgColor =
                            "bg-[#c9983a]/75 border-2 border-[#c9983a]/90";
                          shadowClass =
                            "shadow-[0_3px_14px_rgba(201,152,58,0.45)]";
                        } else if (level >= 3) {
                          bgColor =
                            "bg-gradient-to-br from-[#c9983a] to-[#b8873a] border-2 border-[#ffd700]";
                          shadowClass =
                            "shadow-[0_4px_20px_rgba(201,152,58,0.6),0_0_15px_rgba(255,215,0,0.4)]";
                        }
